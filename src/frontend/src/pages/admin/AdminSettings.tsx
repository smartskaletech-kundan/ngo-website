...
interface TeamPhotosCardProps {
  ...
  treasurerKey?: string;
  onSave: (chairKey: string | null, treasKey: string | null) => Promise<void>;
}
...
function TeamPhotosCard({
  ...
  const [pendingChair, setPendingChair] = useState<string | null>(null);
  const [pendingTreas, setPendingTreas] = useState<string | null>(null);

  async function handleChairUpload(dataUrl: string) {
    const newChair = dataUrl;
    const newTreas = pendingTreas ?? treasurerKey ?? null;
    setPendingChair(newChair);
    await onSave(newChair, newTreas);
  }

  async function handleTreasUpload(dataUrl: string) {
    const newChair = pendingChair ?? chairpersonKey ?? null;
    const newTreas = dataUrl;
    setPendingTreas(newTreas);
    await onSave(newChair, newTreas);
  }
    ...
    >
      <div className="flex items-start gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground font-body mt-1">
            Upload profile photos for the Chairperson and Treasurer. Photos will
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <PhotoUploader
          label="Chairperson"
          initials="CP"
          currentKey={pendingChair ?? chairpersonKey}
          onUpload={handleChairUpload}
          ocidPrefix="settings.chairperson"
        />
        <PhotoUploader
          label="Treasurer"
          initials="TR"
          currentKey={pendingTreas ?? treasurerKey}
          onUpload={handleTreasUpload}
...
export default function AdminSettings() {
  ...
  async function handleSaveTeamPhotos(
    chairKey: string | null,
    treasKey: string | null,
  ) {
    if (!actor) throw new Error("Actor not ready");
    const result = await actor.updateTeamPhotos(chairKey, treasKey);
    if (!result.ok) throw new Error(result.err ?? "Unknown error");
    await refetch();
  }
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground font-body">
                Configure payments and team photos
              </p>
            </div>
          </div>
